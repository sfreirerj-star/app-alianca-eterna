...
# Estilização CSS isolada para os 6 botões principais do painel de navegação
st.markdown(
    """
<style>
    div.stHorizontalBlock > div div.stButton > button {
    ...
    div.stHorizontalBlock > div:nth-child(1) div.stButton > button { background-color: #2563eb !important; } 
    div.stHorizontalBlock > div:nth-child(2) div.stButton > button { background-color: #16a34a !important; } 
    div.stHorizontalBlock > div:nth-child(3) div.stButton > button { background-color: #ca8a04 !important; } 
    div.stHorizontalBlock > div:nth-child(4) div.stButton > button { background-color: #dc2626 !important; } 
    div.stHorizontalBlock > div:nth-child(5) div.stButton > button { background-color: #7c3aed !important; } 
    div.stHorizontalBlock > div:nth-child(6) div.stButton > button { background-color: #0284c7 !important; } 
</style>
""",
    unsafe_allow_html=True,
)

# --- 2. CONTROLE DE LOGIN BLINDADO ---
auth.verificar_autenticacao()

# --- 3. CARREGAMENTO E PROCESSAMENTO DE DADOS ---
df = db.obter_df_processado()
...
if not df.empty:
  ...
  col_nasc1 = db.obter_coluna_segura(
      df, ["Nascimento", "Data de Nascimento"], 2 if len(df.columns) > 2 else 0
  )
  col_endereco = db.obter_coluna_segura(
      df, ["Endereço", "Endereco"], 3 if len(df.columns) > 3 else 0
  )
  col_tel = db.obter_coluna_segura(
      df, ["Telefone", "Celular", "Contato"], 4 if len(df.columns) > 4 else 0
  )
  col_nasc2 = db.obter_coluna_segura(
      df,
      ["Nascimento: 2", "Data de Nascimento: 2", "Nascimento 2"],
      8 if len(df.columns) > 8 else 0,
  )
  ...
  for val in datas_casamento_str:
    ...
    if match_ano:
      ...
      try:
        ...
        pass
    anos_casados_lista.append(
        ano_encontrado
        if (ano_encontrado is not None and 0 <= ano_encontrado <= 80)
        else None
    )
...
# --- 8. OPÇÃO "RELATÓRIOS" ---
  ...
  # --- BLOCO DE RELATÓRIOS DE ACOMPANHAMENTO PASTORAL (EXCLUSIVO NESTA TELA) ---
  st.markdown("---")
